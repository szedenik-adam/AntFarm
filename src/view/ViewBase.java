...
public class ViewBase {
	...
	private static Image loadImgToolkit(String filePath)
	{
		java.net.URL imgUrl = URLClassLoader.getSystemResource(filePath);
		if (imgUrl == null) {
			System.out.println("loadImg URL fail at "+filePath);
			return null;
		}
		Image tankimg = Toolkit.getDefaultToolkit().createImage(imgUrl);
		// Old Image loader (using only filePath without URL - doesn't work with resource files inside jar):
		// Image tankimg = Toolkit.getDefaultToolkit().createImage(new File(filePath).getAbsolutePath());
		int maxAttempt = 10;
		while(tankimg.getHeight(null) == -1 && --maxAttempt>0) {
			try { Thread.sleep(100); } catch (InterruptedException e) { e.printStackTrace(); }
		}
		if(tankimg.getHeight(null) == -1)
		{
			System.out.println("loadImg fail at "+filePath);
			return null;
		}
		return tankimg;
	}

	private static String[] pathPrefixes = new String[] {"images/", "../images/","src/images/","bin/images/", "", "src/","bin/"};
	...
	public static BufferedImage loadImg(String filePath)
	{
		...
		{
			tankimg = loadImgToolkit(pathPrefixes[i]+filePath);
			if(tankimg != null) {
				if (i>0) {String t=pathPrefixes[0]; pathPrefixes[0]=pathPrefixes[i]; pathPrefixes[i]=t;}
				break;
			}
		}
		if(tankimg == null) { System.exit(1); }
		...
		try {
			result=ImageIO.read(new File("images\\anteater.png"));
